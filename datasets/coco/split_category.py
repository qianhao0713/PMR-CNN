...
for dataType in ['instances_train2017.json']: #, 'split_voc_instances_train2017.json']:
    ...
    # for base class, there can be non_voc images
    annotations_split2 = filter_coco(coco, cids_split2)

    # for novel class, there can be voc images
    ...
    split2_file = os.path.join(root_path, 'new_annotations/base_instances_train2017.json')

    with open(split2_file, 'w') as f:
        json.dump(dataset_split2, f)

    split1_file = os.path.join(root_path, 'new_annotations/novel_instances_train2017.json')
