...
class PMMsRCNNv2(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.layer5 = nn.Sequential(
            nn.Conv2d(in_channels=1024 * 2, out_channels=1024, kernel_size=3, stride=1, padding=2, dilation=2,
                      bias=True),
            nn.BatchNorm2d(1024),
            nn.ReLU()
        )
    ...
    def forward(self, batched_inputs):
        ...
        for i in range(B):  # batch
            ...
            pos_support_features = feature_pooled[pos_begin:pos_end].mean(0, True)

            # average pooling support feature for rpn
            pos_support_features_pool = pos_support_features.mean(dim=[2, 3], keepdim=True)

            # PMMs
            pos_prototype_list, pos_Prob_map = self.PMMs(pos_support_features_pool, query_features)

            feature_size = query_features['res4'].shape[-2:]
            for j in range(self.num_pro):

                pos_vec = pos_prototype_list[j]
                # P-Match
                exit_feat_in_ = self.f_v_concate(query_features['res4'], pos_vec, feature_size)
                exit_feat_in_ = self.layer5(exit_feat_in_)
                if j == 0:
                    exit_feat_in = exit_feat_in_
                else:
                    exit_feat_in = exit_feat_in + exit_feat_in_
            exit_feat = self.layer6(exit_feat_in)
            ...
            # rpn
            pos_proposals, pos_anchors, pos_pred_objectness_logits, pos_gt_labels, \
            pos_pred_anchor_deltas, \
            pos_gt_boxes = self.proposal_generator(query_images, pos_features, query_gt_instances)

            # roi_heads
            pos_pred_class_logits, pos_pred_proposal_deltas, pos_detector_proposals = self.roi_heads(query_images,
                                                                                                     query_features,
            ...
            # neg support features from res4, average all supports, for rcnn
            ...
            # PMMs
            neg_prototype_list, neg_Prob_map = self.PMMs(neg_support_features_pool, query_features)

            feature_size = query_features['res4'].shape[-2:]
            for j in range(self.num_pro):

                neg_vec = neg_prototype_list[j]
                # P-Match
                neg_exit_feat_in_ = self.f_v_concate(query_features['res4'], neg_vec, feature_size)
                neg_exit_feat_in_ = self.layer5(neg_exit_feat_in_)
                if j == 0:
                    neg_exit_feat_in = neg_exit_feat_in_
                else:
                    neg_exit_feat_in = neg_exit_feat_in + neg_exit_feat_in_
            neg_exit_feat = self.layer6(neg_exit_feat_in)

            # concat
            neg_concat = torch.cat([neg_exit_feat, neg_Prob_map], dim=1)
            neg_conv = self.layer7(neg_concat)

            neg_features = {'res4': neg_conv}
            neg_support_box_features = support_box_features[neg_begin:neg_end].mean(0, True)

            # rpn
            neg_proposals, neg_anchors, neg_pred_objectness_logits, neg_gt_labels, \
            neg_pred_anchor_deltas, \
            neg_gt_boxes = self.proposal_generator(query_images, neg_features, query_gt_instances)

            # roi_heads
            neg_pred_class_logits, neg_pred_proposal_deltas, neg_detector_proposals = self.roi_heads(query_images,
                                                                                                     query_features,
                                                                                                     neg_support_box_features,
                                                                                                     neg_proposals,
                                                                                                     query_gt_instances)
            ...
            if self.training:
                proposal_losses = self.proposal_generator.losses(
                    outputs_anchors, outputs_pred_objectness_logits, outputs_gt_labels, outputs_pred_anchor_deltas,
                    outputs_gt_boxes)
    ...
    def inference(self, batched_inputs, detected_instances=None, do_postprocess=True):
        ...
        for cls_id, res4_avg in self.support_dict['res4_avg'].items():
            ...
            support_box_features_in = self.layer5(support_box_features)

            # PMMs
            prototype_list, Prob_map = self.PMMs(support_box_features_in, query_features)

            feature_size = query_features['res4'].shape[-2:]
            for j in range(self.num_pro):

                vec = prototype_list[j]
                exit_feat_in_ = self.f_v_concate(query_features['res4'], vec, feature_size)
                exit_feat_in_ = self.layer5(exit_feat_in_)
                if j == 0:
                    exit_feat_in = exit_feat_in_
                else:
        ...
        if do_postprocess:
            ...
            post_results = PMMsRCNNv2._postprocess(results, batched_inputs, images.image_sizes)
